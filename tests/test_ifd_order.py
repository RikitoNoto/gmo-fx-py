...
class TestIFDOrderApi(ApiTestBase):

    def call_api(
        self,
        symbol: IFDOrderApi.Symbol = IFDOrderApi.Symbol.USD_JPY,
        first_side: IFDOrderApi.Side = IFDOrderApi.Side.BUY,
        first_execution_type: IFDOrderApi.ExecutionType = (
            IFDOrderApi.ExecutionType.LIMIT
        ),
        first_size: int = 10000,
        first_price: float = 130,
        second_execution_type: IFDOrderApi.ExecutionType = (
            IFDOrderApi.ExecutionType.LIMIT
        ),
        second_size: int = 10000,
        second_price: float = 145,
    ...
    def create_response(
        ...
        if data is None:
            data = [self.create_ifd_order_data()]
    ...
    def test_check_url(
        ...
        assert url.startswith("https://forex-api.coin.z.com/private/v1/ifdOrder")

    def check_parse_a_data(self, post_mock: MagicMock, **kwargs) -> IFDOrderResponse:
        post_mock.return_value = self.create_response(
            data=[self.create_ifd_order_data(**kwargs)]
        )
        response = self.call_api()
        assert len(response.ifd_orders) == 1
        return response
    ...
    def test_should_get_root_order_id(self, post_mock: MagicMock):
        ...
        assert response.ifd_orders[0].root_order_id == 2536464541
    ...
    def test_should_get_client_order_id(self, post_mock: MagicMock):
        ...
        assert response.ifd_orders[0].client_order_id == "abbb324dff"
    ...
    def test_should_get_timestamp(self, post_mock: MagicMock):
        ...
        assert response.ifd_orders[0].timestamp == datetime(
            2024, 9, 13, 15, 21, 3, 59000, tzinfo=timezone.utc
        )

    @patch("gmo_fx.api.api_base.post")
    def test_should_parse_some_data(self, post_mock: MagicMock) -> IFDOrderResponse:
        post_mock.return_value = self.create_response(
            data=[self.create_ifd_order_data(), self.create_ifd_order_data()]
        )
        response = self.call_api()
        assert len(response.ifd_orders) == 2
    ...
    def test_should_call_api_with_symbol(self, post_mock: MagicMock) -> None:
        for symbol in IFDOrderApi.Symbol:
    ...
    def test_should_call_api_with_first_execution_type(
        ...
        for execution_type in IFDOrderApi.ExecutionType:
    ...
    @patch("gmo_fx.api.api_base.post")
    def test_should_call_api_with_second_execution_type(
        self,
        post_mock: MagicMock,
    ) -> None:
        for execution_type in IFDOrderApi.ExecutionType:
            body = self.check_call_with(post_mock, second_execution_type=execution_type)
            assert body["secondExecutionType"] == execution_type.value
    ...
    def test_should_call_api_with_second_price(self, post_mock: MagicMock) -> None:
        body = self.check_call_with(post_mock, second_price=145.5)
        assert body["secondPrice"] == "145.5"
